...
Potlock is a web game: TypeScript strict, Three.js client in `apps/game`, Colyseus
authoritative rooms in `services/match`, shared protocol in `packages/shared`. The
repo root `CLAUDE.md` wins over anything below. Ignore the Unity, Unreal and Godot
items here, keep the server authoritative (clients send input only), and never add
scope beyond the current build phase.
